$(function(){
  ...
  $("#regin_username").off().bind("input propertychange", function(){ 
    $('.regin-box-error').text('');
			$.ajax({
				type:"post",
				url:"music video_php/register_search.php",
  ...
  $('#to_contactmsg').click(function(){
    if(window.location.search !=""){
      ...
      var url = "../contact message_full/online_message.html?id="+escape(exid)+"&name="+escape(names);
              window.location.href = url;
    }else{
      window.location.href = "../contact message_full/online_message.html";
    }
  })
    //点击关闭登录窗口
  $('.online-login').click(function(){
    $('.regin-box-hide').hide();
    $('#public-heads-mv').hide();
    if(window.location.search ==""){
  ...
  $('#regin-box-regin').on('click',function(e){
    ...
    $('#online-shop').on('click',function(){
      if(sessionStorage.getItem("name") == null && sessionStorage.getItem("password") == null || sessionStorage.getItem("name") == "" && sessionStorage.getItem("password") == ""){
        ...
        window.location.href = 'mv_page1.html';
      }
      })
    ...
    $('#regin-box-submit').click(function(){
      ...
      if(regin_name.length<5 || regin_pwd.length<5){
        ...
        $.ajax({
				type:"post",
				url:"music video_php/register.php",
    ...
    $('#login-box-submit').click(function(){
      $.ajax({
            ...
            if($("input[type='checkbox']").is(':checked')){
              ...
              // $('.online-login').hide();
              // alert($.cookie("name"));
              var url = "mv_page1.html?id="+escape(arr[0]['Id'])+"&name="+escape(arr[0]['user_name']);
              window.location.href = url ;
            }else{
              $.cookie("name", "null");
